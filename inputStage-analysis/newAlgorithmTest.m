...
smoothedData = filter(window,1,elecData);  % Z is the RMS N-channel T-length sEMG signal
Z = smoothedData;
...
% using the NNMF toolbox

optionstruct = struct('beta',10);

[W,F] = sparsenmfnnls(Z,4,optionstruct);
% 
% % let's decompose it into 8 synergies
...
H = [H1;H2;H3;H4];
maxes = max(H.');
W5 = [W1*maxes(1),W2*maxes(2),W3*maxes(3),W4*maxes(4)];  % is this it? did I win?
